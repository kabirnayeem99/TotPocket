...
| A UI/composable callback (or a ViewModel/handler function it calls) does I/O or CPU-heavy work without an explicit background dispatcher | Not a recomposition-axis issue — see `.claude/agents/compose-performance.md` CR-6 (move it off Main, self-contained `withContext`), then `kotlin-coroutines-structured-concurrency` for scope ownership |
